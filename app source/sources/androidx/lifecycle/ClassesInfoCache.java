...
class ClassesInfoCache {
    ...
    private void verifyAndPutHandler(Map<MethodReference, Lifecycle.Event> map, MethodReference methodReference, Lifecycle.Event event, Class cls) {
        ...
        if (event2 != null && newEvent != event2) {
            ...
            Throwable th2 = th;
            new StringBuilder();
            new IllegalArgumentException(sb.append("Method ").append(method.getName()).append(" in ").append(klass.getName()).append(" already declared with different @OnLifecycleEvent value: previous").append(" value ").append(event2).append(", new value ").append(newEvent).toString());
            throw th2;
        } else if (event2 == null) {
