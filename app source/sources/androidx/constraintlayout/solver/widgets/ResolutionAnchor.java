...
public class ResolutionAnchor extends ResolutionNode {
    ...
    public String toString() {
        ...
        if (this.state != 1) {
            new StringBuilder();
            return sb.append("{ ").append(this.myAnchor).append(" UNRESOLVED} type: ").append(sType(this.type)).toString();
        } else if (this.resolvedTarget == this) {
            new StringBuilder();
            return sb3.append("[").append(this.myAnchor).append(", RESOLVED: ").append(this.resolvedOffset).append("]  type: ").append(sType(this.type)).toString();
        } else {
            new StringBuilder();
            return sb2.append("[").append(this.myAnchor).append(", RESOLVED: ").append(this.resolvedTarget).append(Constants.COMMON_SCHEMA_PREFIX_SEPARATOR).append(this.resolvedOffset).append("] type: ").append(sType(this.type)).toString();
        }
    }

    public void resolve(ResolutionAnchor resolutionAnchor, float f) {
        ResolutionAnchor target2 = resolutionAnchor;
        float offset2 = f;
    ...
    public void resolve() {
        ...
        if (this.state != 1 && this.type != 4) {
            ...
            if (this.type == 1 && (this.target == null || this.target.state == 1)) {
                ...
                boolean isEndAnchor = this.myAnchor.mType == ConstraintAnchor.Type.RIGHT || this.myAnchor.mType == ConstraintAnchor.Type.BOTTOM;
                if (isEndAnchor) {
                ...
                if (isEndAnchor) {
                    ...
                    this.resolvedOffset = (this.target.resolvedOffset - ((float) margin)) - (distance3 * (1.0f - percent));
                } else {
                    this.resolvedOffset = this.target.resolvedOffset + ((float) margin) + (distance3 * percent);
                    this.opposite.resolvedOffset = (this.opposite.target.resolvedOffset - ((float) oppositeMargin)) - (distance3 * (1.0f - percent));
                }
                didResolve();
    ...
    public void addResolvedValue(LinearSystem linearSystem) {
        ...
        ArrayRow addEquality = system.addEquality(sv, system.createObjectVariable(this.resolvedTarget.myAnchor), (int) (this.resolvedOffset + 0.5f), 6);
    }
