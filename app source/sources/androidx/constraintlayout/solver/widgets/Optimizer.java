...
public class Optimizer {
    ...
    private static boolean optimizableMatchConstraint(ConstraintWidget constraintWidget, int i) {
        ...
        int orientation = i;
        if (constraintWidget2.mListDimensionBehaviors[orientation] != ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
            return false;
        }
        if (constraintWidget2.mDimensionRatio != 0.0f) {
            if (constraintWidget2.mListDimensionBehaviors[orientation == 0 ? (char) 1 : 0] == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
                return false;
            }
            return false;
        }
        if (orientation == 0) {
            ...
            if (!(constraintWidget2.mMatchConstraintMinWidth == 0 && constraintWidget2.mMatchConstraintMaxWidth == 0)) {
                return false;
            }
        } else if (constraintWidget2.mMatchConstraintDefaultHeight != 0) {
            return false;
        } else {
            if (!(constraintWidget2.mMatchConstraintMinHeight == 0 && constraintWidget2.mMatchConstraintMaxHeight == 0)) {
                return false;
            }
        }
        return true;
    ...
    static void analyze(int optimisationLevel, ConstraintWidget constraintWidget) {
        ConstraintWidget widget = constraintWidget;
    ...
    static boolean applyChainOptimized(ConstraintWidgetContainer container, LinearSystem linearSystem, int i, int i2, ChainHead chainHead) {
        ...
        boolean z2 = container.mListDimensionBehaviors[orientation] == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT;
        if (orientation == 0) {
        ...
        if (numMatchConstraints <= 0 || numMatchConstraints != numVisibleWidgets) {
            ...
            if (isChainPacked) {
                ...
                float distance2 = firstOffset + ((distance - extraMargin) * first.getBiasPercent(orientation));
                while (widget2 != null) {
                ...
                while (widget3 != null) {
                    ...
                    if (LinearSystem.sMetrics != null) {
                        ...
                        LinearSystem.sMetrics.resolvedWidgets++;
                        LinearSystem linearSystem9 = system;
                        LinearSystem.sMetrics.chainConnectionResolved++;
                    }
                    ...
                    if (next3 != null || widget3 == last) {
                        ...
                        distance4 += dimension + ((float) widget3.mListAnchors[offset + 1].getMargin());
            ...
            while (widget4 != null) {
                LinearSystem linearSystem10 = system;
                if (LinearSystem.sMetrics != null) {
                    LinearSystem linearSystem11 = system;
                    LinearSystem.sMetrics.nonresolvedWidgets--;
                    LinearSystem linearSystem12 = system;
                    LinearSystem.sMetrics.resolvedWidgets++;
                    LinearSystem linearSystem13 = system;
                    LinearSystem.sMetrics.chainConnectionResolved++;
                }
    ...
    static void setOptimizedWidget(ConstraintWidget constraintWidget, int i, int resolvedOffset) {
        ConstraintWidget widget = constraintWidget;
        int orientation = i;
