...
public class LruCache<K, V> {
    ...
    public final V get(@NonNull K k) {
        ...
        synchronized (this) {
            try {
                LruCache<K, V> mapValue = this.map.get(key);
                if (mapValue != null) {
                    this.hitCount++;
                    LruCache<K, V> lruCache = mapValue;
                    return lruCache;
                }
                this.missCount++;
                LruCache<K, V> createdValue = create(key);
                ...
                synchronized (this) {
                    try {
                        this.createCount++;
                        LruCache<K, V> mapValue2 = this.map.put(key, createdValue);
                        if (mapValue2 != null) {
    ...
    public final V put(@NonNull K k, @NonNull V v) {
        ...
        synchronized (this) {
            try {
                ...
                LruCache<K, V> previous = this.map.put(key, value);
                if (previous != null) {
    ...
    public final V remove(@NonNull K k) {
        ...
        synchronized (this) {
            try {
                LruCache<K, V> previous = this.map.remove(key);
                if (previous != null) {
                    this.size -= safeSizeOf(key, previous);
                }
                if (previous != null) {
                    entryRemoved(false, key, previous, (LruCache<K, V>) null);
                }
                return previous;
            } catch (Throwable th3) {
                while (true) {
