...
public abstract class Complex extends Quantity {
    ...
    public String toString(int i) {
        ...
        new StringBuilder();
        String imString = sb.append(im().toString(radix)).append("i").toString();
        if (imString.charAt(0) != '-') {
            new StringBuilder();
            imString = sb3.append(Marker.ANY_NON_NULL_MARKER).append(imString).toString();
        }
        if (re().isZero()) {
            return imString;
        }
        new StringBuilder();
        return sb2.append(re().toString(radix)).append(imString).toString();
    }
