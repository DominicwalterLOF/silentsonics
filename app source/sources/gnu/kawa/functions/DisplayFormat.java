...
public class DisplayFormat extends AbstractFormat {
    ...
    public void writeBoolean(boolean z, Consumer out) {
        boolean v = z;
        write(this.language == 'S' ? v ? "#t" : "#f" : v ? RsaJsonWebKey.FACTOR_CRT_COEFFICIENT : "nil", out);
    }
        ...
        if (obj2 instanceof Boolean) {
            ...
            if (showRadix) {
                if (b == 16) {
                    ...
                    new StringBuilder();
                    write(sb.append("#").append(base).append(RsaJsonWebKey.PRIME_FACTOR_OTHER_MEMBER_NAME).toString(), out);
                }
            }
            write(asString2, out);
            ...
            write(Constants.COMMON_SCHEMA_PREFIX_SEPARATOR, out);
    ...
    public void writeSymbol(Symbol symbol, Consumer consumer, boolean z) {
        ...
        if (namespace == Keyword.keywordNamespace) {
            ...
            if (hasUri && (readable2 || !hasPrefix)) {
                out.write(123);
                out.write(uri);
                out.write((int) ErrorLogHelper.MAX_PROPERTY_ITEM_LENGTH);
            }
            out.write(58);
        }
