...
final class BitmapPreFillRunner implements Runnable {
    ...
    private boolean allocate() {
        ...
        while (!this.toPrefill.isEmpty() && !isGcDetected(start)) {
            ...
            if (getFreeMemoryCacheBytes() >= Util.getBitmapByteSize(bitmap)) {
                new UniqueKey((AnonymousClass1) null);
            ...
            if (Log.isLoggable(TAG, 3)) {
                new StringBuilder();
                int d = Log.d(TAG, sb.append("allocated [").append(toAllocate.getWidth()).append(EllipticCurveJsonWebKey.X_MEMBER_NAME).append(toAllocate.getHeight()).append("] ").append(toAllocate.getConfig()).append(" size: ").append(Util.getBitmapByteSize(bitmap)).toString());
            }
        }
    ...
    private boolean isGcDetected(long startTimeMs) {
        return this.clock.now() - startTimeMs >= MAX_DURATION_MS;
    }
    ...
    private static class UniqueKey implements Key {
        ...
        /* synthetic */ UniqueKey(AnonymousClass1 r4) {
            this();
